...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const isMember = room.members.some((m: any) => m.user._id.toString() === session.user.id);
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const room = await Room.findById(params.id);
    if (!room) return apiError("Room not found", 404);
    const member = room.members.find((m: any) => m.user.toString() === session.user.id);
    if (!member || !["owner", "admin"].includes(member.role)) return apiError("Forbidden", 403);
    const body = await req.json();
    const allowed = ["name", "description", "avatar", "banner", "isPublic", "settings", "tags"];
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!isOwner && session.user.role !== "admin") return apiError("Forbidden", 403);
    await Room.findByIdAndDelete(params.id);
    await AuditLog.create({ actor: session.user.id, action: "room_deleted", targetModel: "Room", targetId: params.id, severity: "medium" });
